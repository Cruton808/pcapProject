...
public class CommonUsageExamples {
    ...
    public static void main(String[] args) {
        ...
        try {
            file_out = new PrintWriter(new FileWriter("output/output_test.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        file_out.println("######File output##########");
        ...
        /*
        final int[] udp_count = {0};
        final int[] tcp_count = {0};
        ...
        System.out.println("TCP count is: " + tcp_count[0]);
        System.out.println("UDP count is: " + udp_count[0]);
        */
        ...
        pcap.loop(50, new JPacketHandler<StringBuilder>() {
            ...
            public void nextPacket(JPacket packet, StringBuilder errbuf) {
                ...
                if (packet.hasHeader(Tcp.ID)) {
                    ...
                    finalFile_out.printf("tcp.dst_port=%d%n", tcp.destination());
                    finalFile_out.printf("tcp.src_port=%d%n", tcp.source());
                    finalFile_out.printf("tcp.ack=%x%n", tcp.ack());
                ...
                if (packet.hasHeader(arp)) {
                    System.out.printf("arp header::%s%n", arp.toString());
                    finalFile_out.printf("arp header::%s%n", arp.toString());
                }

                //IPV4
                if (packet.hasHeader(ip4)) {
                    System.out.printf("ip4 header::%s%n", ip4.toString());
                    finalFile_out.printf("ip4 header::%s%n", ip4.toString());
                }

                    //UDP
                if (packet.hasHeader(ip4) && packet.hasHeader(udp)) {
                    System.out.printf("udp header::%s%n", udp);
                    finalFile_out.printf("udp header::%s%n", udp);
                }

                    //TCP
                if (packet.hasHeader(ip4) && packet.hasHeader(tcp)) {
                    System.out.printf("tcp header::%s%n", tcp);
                    finalFile_out.printf("tcp header::%s%n", tcp);
                }

                if (packet.hasHeader(ip4) && packet.hasHeader(tcp) && packet.hasHeader(http)) {
                 ...
                 * the command line.
                 */
                //if (packet.hasHeader(tcp) && packet.hasHeader(http) && packet.hasHeader(ip4)) {
                    /*
                ...
                finalFile_out.printf("frame #%d%n", packet.getFrameNumber());
            }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (packet.hasHeader(tcp)) {
                ...
                finalFile_out.printf("#%d seq=%08X%n", packet.getFrameNumber(), tcp.seq());
            }
        }
        ...
        for (JFlow flow : flows.values()) {
            ...
            if (flow.isReversable()) {
                ...
                for (JPacket p : forward) {
                    ...
                    finalFile_out.printf("%d, ", p.getFrameNumber());
                }
                System.out.println();
        ...
        finalFile_out.printf("superFlowMap::%s%n", superFlowMap);
